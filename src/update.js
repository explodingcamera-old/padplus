...
var checkForMP = function () {
  if (!fs.readdirSync(process.cwd()).length == false) {
    ...
    yesno.ask('Do you want to update MusiqPad? (Y/n)'.prompt, true, function (ok2) {
      if (ok2) {
        dlMusiqPad();
      } else {
        installPadPlus();
...
var dlMusiqPad = function () {
  ...
  download('musiqpad/mqp-server', process.cwd(), function (err) {
    if (err) {
      ...
      console.log('Download finished, now (re)installing dependencies'.info);
      ...
      exec('npm install', { cwd: (process.cwd()) },
        (error, stdout, stderr) => {
          ...
          console.log('Now updating PadPlus stuff...'.info);
          installPadPlus();
        });
    }
  });
...
var installPadPlus = function () {
  ...
  data.plugins.forEach(function (e, index) {
      console.log('Updating '.info + e);
      installPlugin(e, 'plugin', loglevel, function () {
        if (index == data.plugins.length - 1) {
          console.log('Everything was succesfully updated!'.info);
          bundle();
          process.exit();
        }
      });
